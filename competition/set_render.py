...
def template_download(request):
    ...
    file_path = os.path.join(settings.BASE_DIR, settings.TEMPLATE_PATH)  # 希望保留题库文件到一个单独目录
...
def set_game(request):
    ...
    banks = BankInfo.objects.values_list('bank_name', 'bank_id', 'kind_num', 'choice_num', 'fillinblank_num').order_by(
        '-kind_num')[:10]
