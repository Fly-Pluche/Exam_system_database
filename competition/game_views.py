...
def get_questions(request):
    ...
    save_list = ['pk', 'bank_id', 'source', 'ctype', 'qtype','answer']
    question_list = []
    for it in qs_copy:
        question = ''

        if '？' in it['question']:
            [it.pop(k) for k in (save_list + ['media'])]
            for k, v in it.items():
                question += str(v).replace(',', ';')
        else:
            [it.pop(k) for k in save_list]
            for k, v in it.items():
                if k == 'qtype':
                    continue
                else:
                    question = v.replace('##', '_____')
        question_list.append(question)
    qa_info = CompetitionQAInfo.objects.select_for_update().create(  # 创建答题log数据
        kind_id=kind_id,
        uid=uid,
        qsrecord=question_list,
...
def submit_answer(request):
    ...
    total, correct, wrong, state_list,replay_list = check_correct_num(answer)  # 检查答题情况
    qa_info.uid=uid
    qa_info.kind_id=kind_id
    qa_info.qa_id=qa_id
    qa_info.aslogrecord = replay_list
...
def games_history(request):
    ...
    try:
        qa_info=CompetitionQAInfo.objects.get(uid=uid)
    except qa_info.DoesNotExist:
        qa_info = None

    return render(request, 'competition/history_game.html',{'qa_info':qa_info})
